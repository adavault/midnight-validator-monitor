...
**Version:** v1.0.3 (Released 2026-01-25) ✓ Production
...
Systemd integration (generated by `mvm install`):
- `mvm-sync.service` - Continuous sync daemon (Type=simple, Restart=on-failure)
- `mvm-status.service` - One-shot health check (Type=oneshot)
- `mvm-status.timer` - Periodic health checks every 5 minutes
- Installation: `sudo mvm install` - Self-installing binary, creates directories, systemd services, config
- Uninstallation: `sudo mvm install uninstall` - Removes services and symlink (optionally data)
